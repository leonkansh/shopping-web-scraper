def searchAmazon(search, requests, BeautifulSoup, driver):
    print()
    print('Gathering Walmart listings...')
    ...
    # save the price
    walmart_price = walmart_price.text.strip() # call strip() to remove extraneous characters
    walmart_price = walmart_price.replace('$', '') # then we can do math operation/comparsion
    walmart_price = float(walmart_price)

    # print('price is returned')
    return walmart_price, URL
